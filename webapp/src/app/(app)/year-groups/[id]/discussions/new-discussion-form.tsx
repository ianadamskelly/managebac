...
import { createYgDiscussion } from "./actions";

const inputCls =
  "w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500";

export function NewYgDiscussionForm({ yearGroupId }: { yearGroupId: string }) {
  const [open, setOpen] = useState(false);
  const action = createYgDiscussion.bind(null, yearGroupId);
